...
# convert the text to lower cases only and tokenize the text
...
# Ex1_features uses the methodology from exercise 1, 
# The pipeline was not used because we wanted for the svd 
# a matrix conversion 
def Ex1_features(X):
	print("Ex1 features is starting...")
	vectorizer=CountVectorizer(stop_words='english')
	transformer=TfidfTransformer()
	# n_components is set to 20 because we did not want to lose 
	# much info
	svd=TruncatedSVD(n_components=20, random_state=42)
	X_vect=vectorizer.fit_transform(X)
	X_tfidf=transformer.fit_transform(X_vect)
	X_svd=sparse.csr_matrix(svd.fit_transform(X_tfidf))
